...
- Bản đồ thế giới tương tác dùng VIETMAP Web GL: kéo, zoom, fit toàn tuyến, về Việt Nam, 7 địa điểm, đường hải trình, tàu chuyển động, panel chi tiết, điều hướng trước/sau và trạng thái đã khám phá.
...
React 18, TypeScript strict, Vite, React Router, Tailwind CSS, GSAP + ScrollTrigger, Framer Motion, VIETMAP GL JS 6.0.1, SVG fallback nội bộ và Lucide React.
...
│       │   └── journey/        # VIETMAP, lớp chủ quyền và SVG fallback
...
Sao chép `.env.example` nếu cần đổi cổng hoặc origin. Bản đồ VIETMAP cần một tilemap API key dành cho web:
...
Đăng ký tại [VIETMAP Console](https://maps.vietmap.vn/console-v2/register), lấy **tilemap key** (không phải service key), thêm khóa vào `client/.env`, rồi khởi động lại Vite. Theo tài liệu VIETMAP, cần giới hạn key theo domain/referrer, API được phép và hạn mức sử dụng. Biến `VITE_` được trình duyệt đọc nên không phải secret phía server; không commit khóa thật và luôn cấu hình restriction trên Console.

Nếu khóa trống, `/journey` giữ SVG nội bộ làm fallback và hiển thị hướng dẫn cấu hình. Nếu khóa sai hoặc bị chặn domain, giao diện báo lỗi VIETMAP; project không fallback sang OpenStreetMap hay provider thứ ba.
...
- Repository không chứa API key VIETMAP. Vì vậy tile thực tế và nội dung tại từng mức zoom phải được nghiệm thu với key đã giới hạn domain trước khi triển khai chính thức.
...
- Thêm kiểm thử screenshot định kỳ cho các mức zoom chủ quyền khi CI có browser và VIETMAP tilemap key dành riêng cho kiểm thử.

## Screenshot

Không đưa ảnh chụp tự động vào repository vì phiên kiểm thử không có browser renderer khả dụng và không có VIETMAP tilemap key. Giao diện có thể được chụp trực tiếp tại `/journey` sau khi cấu hình key và chạy `npm run dev`.
